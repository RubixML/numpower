--TEST--
NumPower::truncatedNormal() raises clear errors on bad scalars, invalid dtype/device
--FILE--
<?php
...
try {
    NumPower::truncatedNormal([4], 0, 1, 'banana');
...
foreach ([2, -1, 99] as $bad) {
    try {
        NumPower::truncatedNormal([4], 0, 1, 'float32', $bad);
...
foreach (['loc' => [new stdClass(), 1],
    ...
    try {
        NumPower::truncatedNormal([4], $lc, $sc);
...
foreach (['loc' => [-1, 1], 'scale' => [0, -1]] as $tag => [$lc, $sc]) {
    try {
        NumPower::truncatedNormal([4], $lc, $sc, 'uint64');
...
/* After every failure the engine still produces a correct truncatedNormal. */
$ok = NumPower::truncatedNormal([4], 0.0, 1.0, 'float32');
