#ifndef _WIN32
/* config.h is generated by POSIX autoconf from config.h.in and supplies
   HAVE_CUDNN / HAVE_CUBLAS / etc. On Windows the same flags are passed
   to nvcc via -D in build-cuda-windows.bat (PHP's MSVC build uses
   main\config.w32.h, which lives at a different path and isn't reachable
   from this relative include). */
#include "../../../config.h"
#endif
